<?php
    include_once('../config.php');
		...
		$lge = new League();
		$season = $lge->getStartYear();
		
		//check for transactions from the start year
		$sql = "SELECT COUNT(*) FROM fof_transactions_scouts WHERE Season = " . $season;
		$qTransScouts = $dataMgr->runQuery($sql);
		
		if(mysql_num_rows($qTransScouts) == 0){
		...
		//check for transactions from this year
		$sql = "SELECT COUNT(*) FROM fof_transactions_coaches WHERE Season = " . $season;
		$qTransCoaches = $dataMgr->runQuery($sql);
		
		if(mysql_num_rows($qTransCoaches) == 0){
